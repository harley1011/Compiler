...
TEST(FileWithErrorsTest, ParserTests) {
    ...
    Parser parser("built_given_program_with_errors_parser_deriv_out.txt", "built_syntax_with_errors_error_report_out.txt");
...
TEST(aParamsTailTest, ParserTests) {
    ...
    EXPECT_TRUE(parser.aParamsTail());
    EXPECT_EQ(parser.current_rhs_derivation_, ", integer");
}
...
TEST(aParamsTailMultiArgTest, ParserTests) {
    ...
    EXPECT_TRUE(parser.aParamsTail());
    EXPECT_EQ(parser.current_rhs_derivation_, ", integer , float , id [ integer ] , integer + integer * integer / ( integer - integer )");
}
...
TEST(aParamsMultiArgTest, ParserTests) {
    ...
    EXPECT_TRUE(parser.aParams());
    EXPECT_EQ(parser.current_rhs_derivation_, "( integer + integer ) , float , id [ integer ] , integer + integer * integer / ( integer - integer )");
}
...
TEST(varOrFuncIdNestFuncTest, ParserTests) {
    ...
    EXPECT_TRUE(parser.varOrFuncIdNest());
...
TEST(factorNestedFuncTest, ParserTests) {
    ...
    EXPECT_TRUE(parser.factor());
    EXPECT_EQ(parser.current_rhs_derivation_, "id [ integer ] [ integer ] [ integer + integer ] . id ( integer , integer - integer , id . id ( integer , integer ) )");
}
...
TEST(termMultipleTest, ParserTests) {
    ...
    EXPECT_TRUE(parser.term());
    EXPECT_EQ(parser.current_rhs_derivation_, "id * float * ( integer + integer ) / id ( integer , integer ) * id . id [ integer ]");
}


TEST(arithExprAddTest, ParserTests) {
    Parser parser = common_setup("var2 + 10.21;", "<arithExpr>");
    EXPECT_TRUE(parser.arithExpr());
    EXPECT_EQ(parser.current_rhs_derivation_, "id + float");
}
...
TEST(exprGtEqTest, ParserTests) {
    ...
    EXPECT_TRUE(parser.expr());
    EXPECT_EQ(parser.current_rhs_derivation_, "integer * ( integer + id ( integer , integer ) ) - integer >= ( id [ integer ] . id - integer ) * integer");
}
...
TEST(statBlockSingleLineIfTest, ParserTests) {
    Parser parser = common_setup("{ if ( 10 < 20 ) then var7 = 10; else var9 = var6(10, 10); };", "<statBlock>");
        EXPECT_TRUE(parser.statBlock());
    EXPECT_EQ(parser.current_rhs_derivation_, "{ if ( integer < integer ) then id = integer ; else id = id ( integer , integer ) ; } ;");
}

TEST(statBlockMultiLineIfTest, ParserTests) {
    Parser parser = common_setup("{ if ( 10 <  var10(10, 10) ) then {var7 = 10; var10 = 10 * 20 / 20; }else { var9 = var6(10, 10); }; };", "<statBlock>");
    EXPECT_TRUE(parser.statBlock());
    EXPECT_EQ(parser.current_rhs_derivation_, "{ if ( integer < id ( integer , integer ) ) then { id = integer ; id = integer * integer / integer ; } else { id = id ( integer , integer ) ; } ; } ;");
}
TEST(statBlockMultiLineEmptyElseIfTest, ParserTests) {
    Parser parser = common_setup("{ if ( 10 <  var10(10, 10) ) then {var7 = 10; var10 = 10 * 20 / 20; }else { }; };", "<statBlock>");
    EXPECT_TRUE(parser.statBlock());
    EXPECT_EQ(parser.current_rhs_derivation_, "{ if ( integer < id ( integer , integer ) ) then { id = integer ; id = integer * integer / integer ; } else { } ; } ;");
}


TEST(statBlockMultiLineAndSingleLineIfTest, ParserTests) {
    Parser parser = common_setup("{ if ( 10 <  var10(10, 10) ) then var2 = var3; else { var9 = var6(10, 10); }; };", "<statBlock>");
    EXPECT_TRUE(parser.statBlock());
    EXPECT_EQ(parser.current_rhs_derivation_, "{ if ( integer < id ( integer , integer ) ) then id = id ; else { id = id ( integer , integer ) ; } ; } ;");
}
...
TEST(statBlockSingleLineForTest, ParserTests) {
    ...
    EXPECT_TRUE(parser.statBlock());
    EXPECT_EQ(parser.current_rhs_derivation_, "{ for ( int id = integer ; id < integer ; id = id + integer ) id = integer * integer ; } ;");
}

TEST(statBlockMultiLineForTest, ParserTests) {
    Parser parser = common_setup("{ for(int i = 0; i < 12; i = i + 1 ) { count = 18 * 19; count1 = func(10, 20, 10); }; };", "<statBlock>");
    EXPECT_TRUE(parser.statBlock());
    EXPECT_EQ(parser.current_rhs_derivation_, "{ for ( int id = integer ; id < integer ; id = id + integer ) { id = integer * integer ; id = id ( integer , integer , integer ) ; } ; } ;");
...
TEST(funcBodyTest, ParserTests) {
    ...
    EXPECT_EQ(parser.current_rhs_derivation_, "{ id = float * integer / integer - integer ; for ( int id = integer ; id < integer ; id = id + integer ) { if ( id > integer ) then id = id + integer ; else id = id + integer ; } ; return ( id + integer ) ; }");
}

TEST(funcDefTest, ParserTests) {
    Parser parser = common_setup("int varFunc(int a, int b[5], float c){ var1 = 10.21 * 10 / 10 - 12; for (int i = 0; i < 10; i = i + 1) { if ( var2 > 10 ) then var1 = var1 + 10; else var3 = var3 + 10; }; return(var2  + 10); };", "<funcDef>");
    EXPECT_TRUE(parser.funcDef());
    EXPECT_EQ(parser.current_rhs_derivation_, "int id ( int id , int id [ integer ] , float id ) { id = float * integer / integer - integer ; for ( int id = integer ; id < integer ; id = id + integer ) { if ( id > integer ) then id = id + integer ; else id = id + integer ; } ; return ( id + integer ) ; } ;");
}
...
TEST(classBodyTest, ParserTests) {
    Parser parser = common_setup("var varFunc() { int var2; var2 = 2; }; int var4; int var7; int var5Func(int a) { }; }", "<classBody>");
    EXPECT_TRUE(parser.classBody());
    EXPECT_EQ(parser.current_rhs_derivation_, "id id ( ) { int id ; id = integer ; } ; int id ; int id ; int id ( int id ) { } ;");
}
...
TEST(classDeclTest, ParserTests) {
    Parser parser = common_setup("class className { var varFunc() { int var2; var2 = 2; }; int var4; int var7; int var5Func(int a) { }; };", "<classDecl>");
    EXPECT_TRUE(parser.classDecl());
    EXPECT_EQ(parser.current_rhs_derivation_, "class id { id id ( ) { int id ; id = integer ; } ; int id ; int id ; int id ( int id ) { } ; } ;");
}

TEST(classDeclLargeTest, ParserTests) {
    Parser parser = common_setup("class className { int var1[4][5][7][8][9][1][0]; float var2; int findMax(int array[100]) { int maxValue; int idx; maxValue = array[100]; for( int idx = 99; idx > 0; idx = idx - 1 ) { if(array[idx] > maxValue) then { maxValue = array[idx]; }else{}; }; return (maxValue); }; int findMin(int array[100]) { int minValue; int idx; minValue = array[100]; for( int idx = 1; idx <= 99; idx = ( idx ) + 1) { if(array[idx] < maxValue) then { maxValue = array[idx]; }else{}; }; return (minValue); }; };", "<classDecl>");
    EXPECT_TRUE(parser.classDecl());
    EXPECT_EQ(parser.current_rhs_derivation_, "class id { int id [ integer ] [ integer ] [ integer ] [ integer ] [ integer ] [ integer ] [ integer ] ; float id ; int id ( int id [ integer ] ) { int id ; int id ; id = id [ integer ] ; for ( int id = integer ; id > integer ; id = id - integer ) { if ( id [ id ] > id ) then { id = id [ id ] ; } else { } ; } ; return ( id ) ; } ; int id ( int id [ integer ] ) { int id ; int id ; id = id [ integer ] ; for ( int id = integer ; id <= integer ; id = ( id ) + integer ) { if ( id [ id ] < id ) then { id = id [ id ] ; } else { } ; } ; return ( id ) ; } ; } ;");
}


TEST(progTest, ParserTests) {
    Parser parser = common_setup("program {int sample[100]; int idx; int maxValue; int minValue; Utility utility; Utility arrayUtility[2][3][6][7]; for(int t = 0; t<=100 ; t = t + 1) { get(sample[t]); sample[t] = (sample[t] * randomize()); }; maxValue = utility.findMax(sample); minValue = utility.findMin(sample); utility. var1[4][1][0][0][0][0][0] = 10; arrayUtility[1][1][1][1].var1[4][1][0][0][0][0][0] = 2; put(maxValue); put(minValue); };", "<progBody>");
    EXPECT_TRUE(parser.progBody());
    EXPECT_EQ(parser.current_rhs_derivation_, "program { int id [ integer ] ; int id ; int id ; int id ; id id ; id id [ integer ] [ integer ] [ integer ] [ integer ] ; for ( int id = integer ; id <= integer ; id = id + integer ) { get ( id [ id ] ) ; id [ id ] = ( id [ id ] * id ( ) ) ; } ; id = id . id ( id ) ; id = id . id ( id ) ; id . id [ integer ] [ integer ] [ integer ] [ integer ] [ integer ] [ integer ] [ integer ] = integer ; id [ integer ] [ integer ] [ integer ] [ integer ] . id [ integer ] [ integer ] [ integer ] [ integer ] [ integer ] [ integer ] [ integer ] = integer ; put ( id ) ; put ( id ) ; } ;");
}
